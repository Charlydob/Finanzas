...
if ($gBtnIngresosGuardar) $gBtnIngresosGuardar.addEventListener("click", onGuardarIngresosMensuales);
if ($gInputIngresos)      $gInputIngresos.addEventListener("change", onGuardarIngresosMensuales);
if ($gBtnNuevoGasto)      $gBtnNuevoGasto.addEventListener("click", onNuevoGastoFijo);
if ($gBtnCerrarMes)       $gBtnCerrarMes.addEventListener("click", onRegistrarMesGastos);
...
function computeDeltaByAccount(periodo){
  ...
  // ------- Gastos fijos / pestaña nueva -------

function ensureGastosState(){
  if (!state.gastos || typeof state.gastos !== "object"){
    state.gastos = { ...DEFAULT_GASTOS };
  } else {
    if (!Array.isArray(state.gastos.gastosFijos)) state.gastos.gastosFijos = [];
    if (!Array.isArray(state.gastos.historial))   state.gastos.historial   = [];
    if (typeof state.gastos.ingresosMensuales !== "number"){
      state.gastos.ingresosMensuales = esToNumber(state.gastos.ingresosMensuales);
    }
    if (!Number.isFinite(state.gastos.ingresosMensuales)){
      state.gastos.ingresosMensuales = 0;
    }
  }
  console.log("[GASTOS] ensureGastosState", state.gastos);
}

function onGuardarIngresosMensuales(){
  if (!$gInputIngresos) return;

  const raw = $gInputIngresos.value;
  const val = esToNumber(raw);

  console.log("[INGRESOS] Guardar click. raw:", raw, "num:", val);

  ensureGastosState();
  state.gastos.ingresosMensuales = val;

  console.log("[INGRESOS] state.gastos.ingresosMensuales =", state.gastos.ingresosMensuales);

  persistLocal();

  if (window.firebase && state.uid){
    firebase.database()
      .ref(`/users/${state.uid}/finanzas/fase1/gastos/ingresosMensuales`)
      .set(state.gastos.ingresosMensuales)
      .catch(console.error);
  }
  ...
  function computeGastoVariableMesActual(){
    if (!state.registros.length) return { claveMes:null, gastoVariable:0 };
...
function computeGastosMetrics(){
  ensureGastosState();
  const g = state.gastos || DEFAULT_GASTOS;

  const ingresos = Number.isFinite(g.ingresosMensuales)
    ? g.ingresosMensuales
    : esToNumber(g.ingresosMensuales);
  ...
  if (Array.isArray(g.gastosFijos)){
    g.gastosFijos.forEach(item => {
      const imp = esToNumber(item.importe);
      gastosFijos += imp;
      if (item.esencial){
        esenciales += imp;
      } else {
...
function drawGastosSemicircle(ingresos, gastosTotales){
  ...
  console.log("[SEMI] ingresos:", ingresos, "gastosTotales:", gastosTotales);
  ...
  // aquí: parte verde (lo que queda), parte roja (lo gastado)
  const greenShare = 1 - ratio; // p.ej 100 ingresos, 40 gastos -> green=0.6
  const redShare   = ratio;     // -> red=0.4

  const greenAngle = Math.PI * greenShare;
  const redAngle   = Math.PI * redShare;

  // verde desde la izquierda (π) hacia la derecha
  ctx.strokeStyle = "rgba(34,197,94,0.95)";
  ctx.beginPath();
  ctx.arc(cx, cy, radius, Math.PI, Math.PI - greenAngle, true);
  ctx.stroke();

  // rojo desde donde acaba el verde hasta la derecha (0)
  ctx.strokeStyle = "rgba(248,113,113,0.95)";
  ctx.beginPath();
  ctx.arc(cx, cy, radius, Math.PI - greenAngle, 0, true);
  ctx.stroke();
}
  ...
  function renderGastosList(){
    ...
    const items = Array.isArray(g.gastosFijos) ? g.gastosFijos : [];
    ...
    items.forEach((item, idx) => {
      ...
      importeSpan.textContent = numberToEs(esToNumber(item.importe));
  ...
  function handleGastoMenu(idx){
    ...
    item.nombre = nombre;
    item.importe = esToNumber(importeStr);
  ...
  function renderGastosHistorial(){
    ...
    ensureGastosState();
    const hist = Array.isArray(state.gastos.historial)
      ? [...state.gastos.historial]
      : [];
    ...
    hist.forEach((row, idx) => {
      ...
      detalle.textContent =
        `Ing: ${numberToEs(row.ingresos || 0)} · Gastos: ${numberToEs(row.gastosTotales || 0)} · Ahorro: ${pctToEs(pctAhorro)}`;
      main.appendChild(detalle);

      const saldoEl = document.createElement("div");
      saldoEl.className = "g-hist-saldo";
      saldoEl.textContent = numberToEs(row.saldoFinal || 0);
...
function renderGastosPanel(){
  ...
  // actualizar input de ingresos con lo guardado
  if ($gInputIngresos){
    $gInputIngresos.value = ingresos > 0 ? numberToEs(ingresos) : "";
  }

  $gBalanceFinal.textContent = numberToEs(saldoFinal);
  ...
  if ($gBalanceDetalle){
    $gBalanceDetalle.textContent =
      `Ing: ${numberToEs(ingresos)} · Gastos fijos: ${numberToEs(gastosFijos)} · Gasto variable: ${numberToEs(gastoVariable)}`;
  }

  if ($gLabelIngresos) $gLabelIngresos.textContent = numberToEs(ingresos);
  if ($gLabelGastos)   $gLabelGastos.textContent   = numberToEs(gastosFijos);
  ...
  if ($gSemiIngresos) $gSemiIngresos.textContent = numberToEs(ingresos);
  if ($gSemiGastos)   $gSemiGastos.textContent   = numberToEs(gastosTotales);
  if ($gSemiResto){
    $gSemiResto.textContent = numberToEs(saldoFinal);
  ...
  if ($gKpiComprometido){
    ...
    $gKpiComprometido.textContent =
      `${pctToEs(pct)} de tus ingresos está comprometido en gastos fijos.`;
  }
  ...
  if ($gKpiEsencial){
    ...
    $gKpiEsencial.textContent =
      `Esencial: ${pctToEs(pctE)} · Prescindible: ${pctToEs(pctP)}.`;
  }
  ...
  async function syncGastosToCloud(){
    try{
      if (window.firebase && state.uid){
        await firebase.database().ref(`/users/${state.uid}/finanzas/fase1/gastos`).set(state.gastos);
      }
    }catch(e){
      console.error(e);
    }
  ...
  function onNuevoGastoFijo(){
    ...
    const importe = esToNumber(importeStr);
    if (!importe) return;
...
persistLocal();
  ...
  function onRegistrarMesGastos(){
    ...
    const ok = window.confirm(`Registrar el mes ${claveMes} con saldo final ${numberToEs(saldoFinal)}?`);
    if (!ok) return;

    const pctAhorro = ingresos > 0 ? (saldoFinal / ingresos) : 0;

    if (!Array.isArray(state.gastos.historial)){
      state.gastos.historial = [];
    }

    const idx = state.gastos.historial.findIndex(row => row.mes === claveMes);
    const row = { mes:claveMes, ingresos, gastosFijos, gastoVariable, gastosTotales, saldoFinal, pctAhorro };

    if (idx >= 0){
      state.gastos.historial[idx] = row;
    } else {
      state.gastos.historial.push(row);
    }
...
function attachCloudListeners(){
  ...
  cloudRef.on("value", snap => {
    ...
    if (v.gastos && typeof v.gastos === "object"){
      state.gastos = {
        ...DEFAULT_GASTOS,
        ...v.gastos
      };
...
function activarTab(name){
  ...
  if (name === "gastos" && typeof renderGastosPanel === "function"){
    renderGastosPanel();
  }
}
